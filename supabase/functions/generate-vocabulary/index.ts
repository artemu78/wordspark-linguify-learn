...
serve(async (req) => {
  ...
  try {
    const { topic, sourceLanguage, targetLanguage, wordCount = 10 } = await req.json();
    
    const apiKey = 'AIzaSyATFZfhzaC9kBJcLqRk1WGA4PNxtWEsE5Y';
    ...
    return new Response(JSON.stringify({ vocabularyWords }), {
