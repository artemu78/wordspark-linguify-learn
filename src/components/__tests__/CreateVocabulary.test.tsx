...
describe('CreateVocabulary Component', () => {
  ...
  beforeEach(() => {
    ...
    const functionsInvokeMock = supabase.functions.invoke as Mocked<typeof supabase.functions.invoke>; // Use the imported supabase
    functionsInvokeMock.mockImplementation((functionName: string) => {
      if (functionName === 'generate-vocabulary') {
        return Promise.resolve({
          data: {
            vocabularyWords: [{ word: 'auto-word', translation: 'auto-translation' }],
  ...
  test('renders correctly and shows initial form elements', () => {
    ...
    expect(screen.getByRole('button', { name: /Save Vocabulary/i })).toBeInTheDocument();
  ...
  describe('AI Word Generation', () => {
    test('successfully generates word pairs with AI', async () => {
      ...
      await waitFor(() => {
        expect(mockSupabaseClient.functions.invoke).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            body: {
              topic: 'Animals',
              languageYouKnow: 'en', // Default from languageStore mock
              languageToLearn: 'es', // Default from languageStore mock
      ...
      await waitFor(() => {
        ...
        expect(screen.getByPlaceholderText('Translation')).toHaveValue('auto-translation');
      });
    ...
    test('shows error toast if AI generation fails', async () => {
      const user = userEvent.setup();
      // @ts-ignore
      mockSupabaseClient.functions.invoke.mockImplementationOnce((functionName: string) => {
        if (functionName === 'generate-vocabulary') {
          return Promise.resolve({ data: null, error: { message: 'AI failed' } });
        }
        return Promise.resolve({ data: {}, error: null });
      });
    ...
    test('shows error toast if topic is missing for AI generation', async () => {
        ...
        expect(mockSupabaseClient.functions.invoke).not.toHaveBeenCalledWith('generate-vocabulary', expect.anything());
      });
  });
  ...
  describe('Word Translation', () => {
    test('successfully translates a word', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByPlaceholderText('Translation')).toHaveValue('translated-text');
      });
    ...
    test('shows error toast if translation fails', async () => {
      ...
      mockSupabaseClient.functions.invoke.mockImplementationOnce((functionName: string) => {
        if (functionName === 'translate-word') {
          return Promise.resolve({ data: null, error: { message: 'Translation API failed' } });
        }
        return Promise.resolve({ data: {}, error: null });
      });
  ...
  describe.skip('Save Vocabulary', () => {
    ...
    test('successfully saves a new vocabulary', async () => {
      ...
      const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: { id: 'new-vocab-id', title: 'Test Title' }, error: null });
      ...
      (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
        ...
        throw new Error(`Unexpected table ${tableName} in successful save test`);
      });

      const saveButton = screen.getByRole('button', { name: /Save Vocabulary/i });
      await user.click(saveButton);

      await waitFor(() => {
        expect(supabase.from).toHaveBeenCalledWith('vocabularies');
        expect(mockVocabInsertFn).toHaveBeenCalledWith(expect.objectContaining({
          title: 'Test Title',
          topic: 'Test Topic',
      ...
      await waitFor(() => {
         expect(mockSupabaseClient.from).toHaveBeenCalledWith('vocabulary_words');
         expect(mockWordsInsertFn).toHaveBeenCalledWith(expect.arrayContaining([
      ...
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Vocabulary Saved!' }));
    });
    ...
    test('shows error toast if saving vocabulary fails (vocab insert)', async () => {
      ...
      const mockVocabSingleFailFn = vi.fn().mockResolvedValue({ data: null, error: { message: 'Vocab save failed' } });
      const mockVocabSelectFn = vi.fn(() => ({ single: mockVocabSingleFailFn }));
      const mockVocabInsertFn = vi.fn(() => ({ select: mockVocabSelectFn }));
      ...
      (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
        ...
        return { insert: vi.fn(() => { throw new Error("vocabulary_words.insert should not be called in vocab insert fail test"); }) };
      });
    ...
    test('shows error toast if saving vocabulary fails (words insert)', async () => {
        const user = userEvent.setup();
        const mockVocabSingleSuccessFn = vi.fn().mockResolvedValue({ data: { id: 'new-vocab-id', title: 'Test Title' }, error: null });
        const mockVocabSelectSuccessFn = vi.fn(() => ({ single: mockVocabSingleSuccessFn }));
        const mockVocabInsertSuccessFn = vi.fn(() => ({ select: mockVocabSelectSuccessFn }));

        const mockWordsInsertFailFn = vi.fn().mockResolvedValue({ error: { message: 'Words save failed' } });
        ...
        (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
          ...
          throw new Error(`Unexpected table ${tableName} in words insert fail test`);
        });
    ...
    test('shows validation error if no word pairs are added', async () => {
      ...
      test('shows validation error if languages are not selected or are the same', async () => {
        ...
        (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((table: string) => {
            if (table === 'vocabularies') return { insert: vi.fn(() => ({ select: mockSelectFn })) };
            if (table === 'vocabulary_words') return { insert: mockSaveFn };
            // This path should ideally not be hit if validation works.
            throw new Error (`Unexpected Supabase call in language validation test: ${table}`);
        });
  ...
  describe.skip('Create Story', () => {
    beforeEach(async () => {
      ...
      await user.type(screen.getAllByPlaceholderText('Translation')[0], 'storytranslation');

      // Mock successful save for vocab and words for this beforeEach
      const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: { id: 'story-vocab-id', title: 'Story Vocab' }, error: null });
      const mockVocabSelectFn = vi.fn(() => ({ single: mockVocabSingleFn }));
      const mockVocabInsertFn = vi.fn(() => ({ select: mockVocabSelectFn }));
      const mockWordsInsertFn = vi.fn().mockResolvedValue({ error: null });
      ...
      (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
        ...
        if (tableName === 'vocabulary_words') return { insert: mockWordsInsertFn };
        throw new Error(`Unexpected table ${tableName} in Create Story beforeEach`);
      });
    ...
    test('successfully creates a story', async () => {
      ...
      expect(screen.getByText('Story created for this vocabulary.')).toBeInTheDocument();
    ...
    test('shows error toast if story creation fails', async () => {
      const user = userEvent.setup();
      mockGenerateAndSaveStory.mockRejectedValueOnce(new Error('Story gen failed'));
  ...
  test.skip('calls onBack when "Back" or "Done / Back to List" button is clicked', async () => {
    ...
    const mockVocabSingleFnDone = vi.fn().mockResolvedValue({ data: { id: 'done-vocab-id', title: 'Done Vocab' }, error: null });
    const mockVocabSelectFnDone = vi.fn(() => ({ single: mockVocabSingleFnDone }));
    const mockVocabInsertFnDone = vi.fn(() => ({ select: mockVocabSelectFnDone }));
    const mockWordsInsertFnDone = vi.fn().mockResolvedValue({ error: null });

    (supabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
      if (tableName === 'vocabularies') return { insert: mockVocabInsertFnDone };
      if (tableName === 'vocabulary_words') return { insert: mockWordsInsertFnDone };
    ...
    await waitFor(() => {
        expect(screen.getByRole('button', { name: /Done \/ Back to List/i})).toBeInTheDocument();
    });
    const doneButton = screen.getByRole('button', { name: /Done \/ Back to List/i });
