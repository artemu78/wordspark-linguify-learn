...
const mockUseLanguageStore = useLanguageStore as Mocked<typeof useLanguageStore>;
const mockUseToast = useToast as Mocked<typeof useToast>;
const mockGenerateAndSaveStory = generateAndSaveStory as Mocked<typeof generateAndSaveStory>;
...
describe('EditVocabulary Component', () => {
  ...
  const mockWordsData = [
    { id: 'word-1', vocabulary_id: testVocabularyId, word: 'hello', translation: 'hola' },
    { id: 'word-2', vocabulary_id: testVocabularyId, word: 'world', translation: 'mundo' },
  ];
  ...
  beforeEach(() => {
    ...
    const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: mockVocabularyData, error: null });
    const mockVocabEqFn = vi.fn(() => ({ single: mockVocabSingleFn }));
    const mockVocabSelectFn = vi.fn((selectArg) => ({ // Accept select string
    ...
    const mockWordsEqFn = vi.fn().mockResolvedValue({ data: mockWordsData, error: null });
    const mockWordsSelectFn = vi.fn(() => ({ eq: mockWordsEqFn }));

    // Specific mocks for initial data loading via useQuery
    const mockVocabLoadFn = vi.fn().mockResolvedValue({ data: mockVocabularyData, error: null });
    const mockWordsLoadFn = vi.fn().mockResolvedValue({ data: mockWordsData, error: null });

    (supabase.from as MockedFunction<typeof supabase.from>).mockImplementation((tableName: string) => {
      if (tableName === 'vocabularies') {
        return {
          select: vi.fn((selectString) => ({ // select('*, stories (id)')
            eq: vi.fn((col, val) => ({       // .eq('id', vocabularyId)
      ...
      if (tableName === 'vocabulary_words') {
        return {
          select: vi.fn((selectString) => ({ // select('*')
      ...
      return {
        ...
        update: vi.fn().mockReturnThis(),
        eq: vi.fn().mockReturnThis(),
        single: vi.fn().mockResolvedValue({ data: {fallback: true}, error: null }),
      } as any;
    });

    const functionsInvokeMock = supabase.functions.invoke as Mocked<typeof supabase.functions.invoke>;
    functionsInvokeMock.mockImplementation((functionName: string) => {
        if (functionName === 'generate-vocabulary') {
          return Promise.resolve({
            data: {
              vocabularyWords: [{ word: 'ai-gen-word', translation: 'ai-gen-translation' }],
  ...
  test.only('loads and displays vocabulary data correctly', async () => {
    render(
      <AllTheProviders>
  ...
  test('updates title, topic, and isPublic switch', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('Title')).not.toBeDisabled()); // Wait for loading to finish
  ...
  test('adds, removes, and updates word pairs', async () => {
    const user = userEvent.setup();
    render(
      <AllTheProviders>
        <EditVocabulary vocabularyId={testVocabularyId} onBack={mockOnBack} />
    ...
    await waitFor(() => { // Wait for initial words to load
    ...
    const firstTranslationInput = screen.getAllByPlaceholderText('Translation')[0];
    ...
    await waitFor(() => {
        expect(screen.getAllByPlaceholderText('Word')).toHaveLength(mockWordsData.length + 1);
    });
    const newWordInput = screen.getAllByPlaceholderText('Word')[mockWordsData.length];
    const newTranslationInput = screen.getAllByPlaceholderText('Translation')[mockWordsData.length];
    ...
    await waitFor(() => {
      ...
      expect(screen.getAllByPlaceholderText('Word')).toHaveLength(mockWordsData.length); // Initial 2, remove 1, add 1 = 2
  ...
  describe('AI Word Generation and Translation in Edit Mode', () => {
    test('successfully generates new word pairs with AI, replacing existing ones', async () => {
      const user = userEvent.setup();
      render(
        <AllTheProviders>
          <EditVocabulary vocabularyId={testVocabularyId} onBack={mockOnBack} />
      ...
      await waitFor(() => {
        ...
        expect(screen.getByPlaceholderText('Translation')).toHaveValue('ai-gen-translation');
      });
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Success!' }));
    });

    test('successfully translates a word in edit mode', async () => {
      const user = userEvent.setup();
      render(
        <AllTheProviders>
          <EditVocabulary vocabularyId={testVocabularyId} onBack={mockOnBack} />
      ...
      await waitFor(() => {
        ...
        expect(screen.getAllByPlaceholderText('Translation')[0]).toHaveValue('ai-translated');
      });
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Translation Complete' }));
    });
  });

  describe.skip('Update Vocabulary', () => {
    const fillAndSubmitUpdateForm = async (user: ReturnType<typeof userEvent.setup>) => {
      ...
      await user.type(screen.getAllByPlaceholderText('Word')[0], 'updatedword1');

      await user.click(screen.getByRole('button', { name: /Update Vocabulary/i }));
    };

    test('successfully updates a vocabulary', async () => {
      const user = userEvent.setup();
      ...
      mockSupabaseClient.from.mockImplementation((tableName: string) => {
        if (tableName === 'vocabularies') {
          return {
            ...
            update: jest.fn((updateObj) => ({ // For update
            ...
            single: jest.fn().mockResolvedValue({ data: mockVocabularyData, error: null }), // For initial load
          };
        }
        if (tableName === 'vocabulary_words') {
          return {
            select: jest.fn().mockReturnThis(), // For initial load
            eq: jest.fn().mockResolvedValue({ data: mockWordsData, error: null }), // For initial load .eq()
            delete: jest.fn(() => ({ // For delete
              eq: mockWordDeleteFn,
            })),
    ...
    test('shows error toast if updating vocabulary fails (vocab update error)', async () => {
      const user = userEvent.setup();
       // @ts-ignore
      const vocabUpdateErrorMock = jest.fn().mockResolvedValue({ error: { message: 'Vocab DB update failed' } });
       // @ts-ignore
      mockSupabaseClient.from.mockImplementation((tableName: string) => {
        if (tableName === 'vocabularies') {
            if (mockSupabaseClient.from(tableName).select) { // Initial load
                 // @ts-ignore
                return { select: jest.fn(() => ({ eq: jest.fn(() => ({ single: jest.fn().mockResolvedValue({ data: mockVocabularyData, error: null }) })) })) };
            } // Update
             // @ts-ignore
            return { update: jest.fn(() => ({ eq: jest.fn(() => ({ eq: vocabUpdateErrorMock })) })) };
        }
        if (tableName === 'vocabulary_words') { // Initial load for words
             // @ts-ignore
            return { select: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ data: mockWordsData, error: null }) })) };
        }
        return jest.requireActual('@/integrations/supabase/client').supabase.from(tableName);
      });
    ...
    test('shows error toast if updating vocabulary fails (words delete error)', async () => {
        const user = userEvent.setup();
         // @ts-ignore
        mockSupabaseClient.from.mockImplementation((tableName: string) => {
          if (tableName === 'vocabularies') { // Vocab load and update success
            // @ts-ignore
            if (mockSupabaseClient.from(tableName).select) return { select: jest.fn(() => ({ eq: jest.fn(() => ({ single: jest.fn().mockResolvedValue({ data: mockVocabularyData, error: null }) })) })) };
            // @ts-ignore
            return { update: jest.fn(() => ({ eq: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ error: null }) })) })) };
          }
          if (tableName === 'vocabulary_words') {
            // @ts-ignore
            if (mockSupabaseClient.from(tableName).select) return { select: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ data: mockWordsData, error: null }) })) }; // Load
            // @ts-ignore
            return { // Words delete fails
              delete: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ error: { message: 'Words delete failed' } }) })),
    ...
    test('shows validation error if title or topic is missing on update', async () => {
      const user = userEvent.setup();
      render(
        <AllTheProviders>
          <EditVocabulary vocabularyId={testVocabularyId} onBack={mockOnBack} />
    ...
    test('shows validation error if no word pairs are valid on update', async () => {
        const user = userEvent.setup();
        render(
          <AllTheProviders>
            <EditVocabulary vocabularyId={testVocabularyId} onBack={mockOnBack} />
  ...
  describe.skip('Story Management in Edit Mode', () => {
    test('successfully creates a story if none exists', async () => {
      const user = userEvent.setup();
      // Mock initial vocabulary data to have no story
      const noStoryVocabularyData = { ...mockVocabularyData, stories: [] };
       // @ts-ignore
      mockSupabaseClient.from.mockImplementation((tableName: string) => {
        if (tableName === 'vocabularies') { // For initial load
          return { select: jest.fn(() => ({ eq: jest.fn(() => ({ single: jest.fn().mockResolvedValue({ data: noStoryVocabularyData, error: null }) })) })) };
        }
        if (tableName === 'vocabulary_words') { // For initial load
          return { select: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ data: mockWordsData, error: null }) })) };
        }
        return jest.requireActual('@/integrations/supabase/client').supabase.from(tableName);
      });
      ...
      await waitFor(() => { // Wait for data to load and "Create Story" button to appear
        expect(screen.getByRole('button', { name: /Create Story for this Vocabulary/i })).toBeInTheDocument();
      });

      const createStoryButton = screen.getByRole('button', { name: /Create Story for this Vocabulary/i });
      await user.click(createStoryButton);

      await waitFor(() => {
        expect(mockGenerateAndSaveStory).toHaveBeenCalledWith(testVocabularyId);
      });
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Story Created!' }));
    ...
    test('successfully re-generates an existing story', async () => {
      const user = userEvent.setup();
      ...
      await waitFor(() => {
        expect(screen.getByRole('button', { name: /Re-generate Story/i })).toBeInTheDocument();
      });
      const regenerateButton = screen.getByRole('button', { name: /Re-generate Story/i });
      await user.click(regenerateButton);

      await waitFor(() => {
        expect(deleteStoryBitsMock).toHaveBeenCalledWith('story_id', mockVocabularyData.stories[0].id);
        expect(deleteStoryMock).toHaveBeenCalledWith('id', mockVocabularyData.stories[0].id);
        expect(mockGenerateAndSaveStory).toHaveBeenCalledWith(testVocabularyId);
      });
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Old story deleted' }));
      expect(mockToast).toHaveBeenCalledWith(expect.objectContaining({ title: 'Story Re-generated!' }));
    });

    test('shows error if re-generating story fails (e.g., delete old story fails)', async () => {
        const user = userEvent.setup();
        const deleteStoryErrorMock = jest.fn().mockResolvedValue({ error: { message: 'Failed to delete story' } });
         // @ts-ignore
        mockSupabaseClient.from.mockImplementation((tableName: string) => {
          if (tableName === 'vocabularies') return { select: jest.fn(() => ({ eq: jest.fn(() => ({ single: jest.fn().mockResolvedValue({ data: mockVocabularyData, error: null }) })) })) };
          if (tableName === 'vocabulary_words') return { select: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ data: mockWordsData, error: null }) })) };
          if (tableName === 'stories') return { delete: jest.fn(() => ({ eq: deleteStoryErrorMock })) };
          if (tableName === 'story_bits') return { delete: jest.fn(() => ({ eq: jest.fn().mockResolvedValue({ error: null }) })) }; // bits delete fine
          return jest.requireActual('@/integrations/supabase/client').supabase.from(tableName);
        });
        ...
        await waitFor(() => expect(screen.getByRole('button', { name: /Re-generate Story/i })).toBeInTheDocument());
        await user.click(screen.getByRole('button', { name: /Re-generate Story/i }));
  ...
  test('calls onBack when "Back" button is clicked', async () => {
    ...
    await waitFor(() => expect(screen.getByLabelText('Title')).not.toBeDisabled()); // Wait for load
