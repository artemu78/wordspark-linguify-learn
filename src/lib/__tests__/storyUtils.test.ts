...
vi.mock('@/integrations/supabase/client', () => {
  ...
  return {
    supabase: {
      from: actualFromMock,
      functions: {
        invoke: vi.fn().mockResolvedValue({ data: { selfContainedInvoke: true }, error: null }),
      },
      auth: { // Basic auth mock if needed by any indirect imports
        getUser: vi.fn().mockResolvedValue({ data: { user: {id: 'self-contained-user'} }, error: null }),
      },
      rpc: vi.fn().mockResolvedValue({data: {selfContainedRpc: true}, error: null})
    }
  };
});
...
import { generateStoryFromWords, GeminiGenerationError } from '@/lib/geminiUtils';
...
const mockGenerateStoryFromWords = generateStoryFromWords as Mocked<typeof generateStoryFromWords>; // also mocked
...
describe('generateAndSaveStory', () => {
  ...
  const mockGeminiBits = [
    { word: 'hello', storyBitDescription: 'Story about hello', storyBitDescriptionInLanguageYouKnow: 'Historia sobre hola', imagePrompt: 'Prompt for hello' },
    { word: 'world', storyBitDescription: 'Story about world', storyBitDescriptionInLanguageYouKnow: 'Historia sobre mundo', imagePrompt: 'Prompt for world' },
  ];
  ...
  test('successfully generates and saves a story', async () => {
    ...
    const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: mockVocabulary, error: null });
    const mockVocabEqFn = vi.fn(() => ({ single: mockVocabSingleFn }));
    const mockVocabSelectFn = vi.fn(() => ({ eq: mockVocabEqFn }));

    const mockWordsEqFn = vi.fn().mockResolvedValue({ data: mockWords, error: null });
    const mockWordsSelectFn = vi.fn(() => ({ eq: mockWordsEqFn }));

    const mockStorySingleFn = vi.fn().mockResolvedValue({ data: { id: mockNewStoryId }, error: null });
    const mockStorySelectAfterInsertFn = vi.fn(() => ({ single: mockStorySingleFn }));
    const mockStoryInsertFn = vi.fn(() => ({ select: mockStorySelectAfterInsertFn }));

    const mockStoryBitsInsertFn = vi.fn().mockResolvedValue({ error: null });

    (mockSupabase.from as MockedFunction<typeof supabase.from>).mockImplementation((tableName: string) => {
      if (tableName === 'vocabularies') return { select: mockVocabSelectFn };
      if (tableName === 'vocabulary_words') return { select: mockWordsSelectFn };
      if (tableName === 'stories') return { insert: mockStoryInsertFn };
      if (tableName === 'story_bits') return { insert: mockStoryBitsInsertFn };
    ...
    expect(mockVocabSelectFn).toHaveBeenCalledWith('title, source_language, target_language');
    ...
    // @ts-ignore
    const storyBitsInsertArg = mockSupabase.from('story_bits').insert.mock.calls[0][0];
    ...
    expect(storyBitsInsertArg[0]).toEqual(expect.objectContaining({
      ...
      sentence_language_you_know: mockGeminiBits[0].storyBitDescriptionInLanguageYouKnow,
  ...
  test('throws StoryGenerationError if fetching vocabulary details fails', async () => {
    const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: null, error: { message: 'DB error' } });
    const mockVocabEqFn = vi.fn(() => ({ single: mockVocabSingleFn }));
    const mockVocabSelectFn = vi.fn(() => ({ eq: mockVocabEqFn }));

    const mockSingleFn = vi.fn().mockResolvedValue({ data: null, error: { message: 'DB error' } });
    (mockSupabase.from as MockedFunction<typeof supabase.from>).mockImplementationOnce((tableName: string) => {
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Failed to fetch vocabulary details: DB error"),
      })
    );
  ...
  test('throws StoryGenerationError if vocabulary is not found', async () => {
    const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: null, error: null }); // Vocab not found
    const mockVocabEqFn = vi.fn(() => ({ single: mockVocabSingleFn }));
    const mockVocabSelectFn = vi.fn(() => ({ eq: mockVocabEqFn }));
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toMatchObject({ code: "VOCAB_NOT_FOUND" });
  });

  test.skip('throws StoryGenerationError if fetching words fails', async () => {
    const fromSpy = vi.spyOn(mockSupabase, 'from');
    const mockWordsEqFailFn = vi.fn().mockResolvedValue({ data: null, error: { message: 'Words DB error' } });

    fromSpy.mockImplementationOnce((tableName: string) => {
        if (tableName === 'vocabularies') {
            const vocabSingleSuccess = vi.fn().mockResolvedValue({ data: mockVocabulary, error: null });
            return { select: vi.fn(() => ({ eq: vi.fn(() => ({ single: vocabSingleSuccess })) })) };
        }
        ...
        console.error(`Unexpected table call in 'fetching words fails' test: ${tableName}`);
        return {
            select: vi.fn(() => ({
                eq: vi.fn(() => { throw new Error(`Unexpected .eq call for ${tableName}`) }),
                single: vi.fn(() => { throw new Error(`Unexpected .single call for ${tableName}`) })
            })),
            insert: vi.fn(() => { throw new Error(`Unexpected .insert call for ${tableName}`) })
        } as any;
    });
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
        expect.objectContaining({ name: "StoryGenerationError", code: "FETCH_WORDS_FAILED"})
    );
  ...
  test.skip('throws StoryGenerationError if no words are found for vocabulary', async () => {
    const fromSpy = vi.spyOn(mockSupabase, 'from');
    const mockWordsEqNoDataFn = vi.fn().mockResolvedValue({ data: [], error: null }); // No words found
    ...
    fromSpy.mockImplementationOnce((tableName: string) => {
        ...
        console.error(`Unexpected table call in 'no words found' test: ${tableName}`);
        return {
            select: vi.fn(() => ({
                eq: vi.fn(() => { throw new Error(`Unexpected .eq call for ${tableName}`) }),
                single: vi.fn(() => { throw new Error(`Unexpected .single call for ${tableName}`) })
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
        expect.objectContaining({ name: "StoryGenerationError", code: "NO_WORDS_FOUND"})
    );
  ...
  test('throws StoryGenerationError if languages are missing from vocabulary', async () => {
    const vocabWithoutLanguages = { ...mockVocabulary, source_language: null, target_language: null }; // Key change for this test

    const mockVocabSingleFn = vi.fn().mockResolvedValue({ data: vocabWithoutLanguages, error: null });
    const mockVocabEqFn = vi.fn(() => ({ single: mockVocabSingleFn }));
    const mockVocabSelectFn = vi.fn(() => ({ eq: mockVocabEqFn }));

    const mockWordsEqFn = vi.fn().mockResolvedValue({ data: mockWords, error: null }); // Words are fine
    const mockWordsSelectFn = vi.fn(() => ({ eq: mockWordsEqFn }));
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
        expect.objectContaining({ name: "StoryGenerationError", code: "LANGUAGES_MISSING"})
    );
  });
  ...
  test('throws StoryGenerationError if generateStoryFromWords (Gemini) fails', async () => {
    ...
    mockGenerateStoryFromWords.mockRejectedValueOnce(new GeminiGenerationError("Gemini API error", { detail: "some detail" }));
  ...
  test.skip('throws StoryGenerationError if creating story entry in Supabase fails', async () => {
    ...
    const fromSpy = vi.spyOn(mockSupabase, 'from');
    const storySingleFailMock = vi.fn().mockResolvedValue({ data: null, error: { message: 'Story insert DB error' } });
    ...
    fromSpy.mockImplementationOnce((tableName: string) => {
        ...
        if (tableName === 'vocabulary_words') { // Words success
            const wordsEq = vi.fn().mockResolvedValue({ data: mockWords, error: null });
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
        expect.objectContaining({name: "StoryGenerationError", code: "STORY_CREATION_FAILED"})
    );
  ...
  test.skip('throws StoryGenerationError if inserting story bits fails and attempts cleanup', async () => {
    ...
    const vocabSingleSuccess = vi.fn().mockResolvedValue({ data: mockVocabulary, error: null });
    const wordsEqSuccess = vi.fn().mockResolvedValue({ data: mockWords, error: null });
    const storyInsertSingleSuccess = vi.fn().mockResolvedValue({ data: { id: mockNewStoryId }, error: null });
    const storyDeleteEqSuccess = vi.fn().mockResolvedValue({ error: null }); // For cleanup
    const storyBitsInsertFail = vi.fn().mockResolvedValue({ error: { message: 'Bits insert DB error' } }); // Actual fail point
    ...
    fromSpy.mockImplementationOnce((tableName: string) => {
        ...
        if (tableName === 'stories') {
            return {
                insert: vi.fn(() => ({ select: vi.fn(() => ({ single: storyInsertSingleSuccess })) })),
    ...
    await expect(generateAndSaveStory(vocabularyId)).rejects.toThrowError(
        expect.objectContaining({name: "StoryGenerationError", code: "BITS_INSERTION_FAILED"})
    );
